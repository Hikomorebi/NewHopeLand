...
5. **SQL准确性**：当用户询问的问题中的指标存在计算模板时，请完完全全地按照提供的计算规则模板来设计SQL语句，不要无端自行增删或修改计算规则，同时需要提取相关的时间等信息来填充计算规则以生成完整正确的SQL语句。

6. **SQL规范性**：生成的SQL语句不能涵盖非法字符如"\n"，请确保生成的SQL语句能直接在数据库上执行。

7. **时间范围**：请确保SQL语句能够涵盖用户请求的时间范围。如果用户请求的是一段时间内的数据（如从2023年10月1日至2024年11月3日），请确保SQL语句能够正确提取这段时间内的数据。  
...
mategen = MateGen(api_key=os.getenv("OPENAI_API_KEY"),
                  ...
                  system_content_list=[system_prompt],
