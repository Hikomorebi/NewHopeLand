...
def get_translate_column_names(column_names):
    ...
    response = client.chat.completions.create(
        model='qwen2.5-72b-instruct', messages=[{"role":"user","content":final_prompt}]
    )
...
def connect_to_db():
    conn = pymysql.connect(host='8.137.93.112', port=3306, user='new_hope', password='nA48AAse3kCpChnh', db='new_hope_estate_db')
    cur = conn.cursor()
    return conn,cur
...
def process_user_input(user_question):
    ...
    if preset_sql:
        ...
        # 如果没有找到干预问题，进行指标匹配
        matched_indicators = match_indicators(user_question)
        if matched_indicators:
            ...
            # 如果没有找到匹配的指标，进行同义词替换
            modified_question = replace_synonyms(user_question, synonyms)  # 传入同义词字典
