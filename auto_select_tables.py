...
tables_info = {
    "fdc_ads.ads_salesreport_subscranalyse_a_min": {
        ...
        """,
        "fields": [
        "统计日期", "组织类型", "组织编码", "组织名称", "时间范围编码",
        "认购未签金额", "认购未签金额权益后", "认购未签套数", "认购转签约套数",
        "认购转签约平均周期", "首访到认购套数", "首访到认购平均周期", "通路",
        "周期类型", "总来访", "首访", "复访", "新增认购"
        ]
    },
...
def select_table_based_on_query(query, tables_info, client, model):
    # 构建思维链 prompt
    prompt = f"基于用户的查询，选择相关的数据表以进行数据检索：\n\n查询：{query}\n\n数据表信息：\n"
    for table_name, info in tables_info.items():
        prompt += f"\n数据表名称：{table_name}\n数据表解释：{info['description']}数据表包含的字段：{info['fields']}\n"

    prompt += "\n请基于以下步骤来选择适合的数据表：\n"
    prompt += "1. 分析查询中涉及的主要信息（例如，特定的项目名称、年份、所需的信息字段）。\n"
    prompt += "2. 请根据每个数据表的字段信息，判断查询所需的信息字段是否存在，选择那些包含完全一致的字段的表。\n"
    prompt += "3. 根据其他信息，从选择出的表格中二次筛选出可能相关的表格。\n"
    prompt += "4. 考虑剩余的每个表格的信息，选择最合适的一个或多个表格。\n"
    prompt += "5. 输出选择的数据表，以list的形式返回。\n"
...
# 示例查询
query = "查询成都皇冠湖壹号在2017年的建筑面积是多少，给出具体值"
selected_table = select_table_based_on_query(query, tables_info, client, model)
